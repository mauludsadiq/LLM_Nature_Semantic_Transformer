...
fn main() -> Result<()> {
        ...
        let ops = if let Some(ops_array) = json_value.get("ops").and_then(|v| v.as_array()) {
            ...
            for opv in ops_array {
                let op = opv
                    .get("op")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| anyhow!("op missing op"))?;
                match op {
                    "SELECT_UNIVERSE" => {
                        let u = opv
                            .get("universe")
                            .and_then(|v| v.as_str())
                            .ok_or_else(|| anyhow!("SELECT_UNIVERSE missing universe"))?;
                        let n = opv
                            .get("n")
                            .and_then(|v| v.as_u64())
                            .ok_or_else(|| anyhow!("SELECT_UNIVERSE missing n"))?;
                    ...
                    "FILTER_WEIGHT" => {
                        let min = opv
                            .get("min")
                            .and_then(|v| v.as_u64())
                            .ok_or_else(|| anyhow!("FILTER_WEIGHT missing min"))?;
                        let max = opv
                            .get("max")
                            .and_then(|v| v.as_u64())
                            .ok_or_else(|| anyhow!("FILTER_WEIGHT missing max"))?;
                    ...
                    "START_ELEM" => {
                        let elem = opv
                            .get("elem")
                            .and_then(|v| v.as_str())
                            .ok_or_else(|| anyhow!("START_ELEM missing elem"))?;
                    ...
                    "SET_BIT" => {
                        let i = opv
                            .get("i")
                            .and_then(|v| v.as_u64())
                            .ok_or_else(|| anyhow!("SET_BIT missing i"))?;
                        let b = opv
                            .get("b")
                            .and_then(|v| v.as_u64())
                            .ok_or_else(|| anyhow!("SET_BIT missing b"))?;
    ...
    let reference = if let Some(start_op) = trace_ops.iter().find(|op| op.starts_with("LOAD ")) {
        start_op
            ...
            .unwrap_or("13/37")
            .to_string()
    } else if let Some(pop) = trace_ops
        .iter()
        .find(|op| op.starts_with("PROJECT_SIGNATURE"))
    {
        extract_kv(pop, "elem").unwrap_or_else(|| "13/37".to_string())
    } else if let Some(wop) = trace_ops
        .iter()
        .find(|op| op.starts_with("WITNESS_NEAREST"))
    {
